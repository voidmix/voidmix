...
export interface ProjectApplication {
  ...
  listActivity(input: { actorId: string; projectId?: string }): Promise<ActivityV2[]>;
}
  ...
  return {
    ...
    async listActivity({ actorId, projectId }) {
      ...
      if (projectId) {
        ...
        if (
          !project ||
          !canProjectCapabilityV2(await accessFor(actorId, project), "project.read")
        ) {
          throw new ProjectV2DomainError("PROJECT_ACCESS_DENIED", "Activity access denied.");
        }
        return options.activity.listByProject(projectId);
      }
      const projects = await this.listForUser(actorId);
      return (
