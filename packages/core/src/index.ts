...
export interface ProjectRepository {
  ...
  update(input: { id: string; actorId: string; name?: string; description?: string; status?: ProjectStatus }): Promise<Project>;
  ...
  updateTask(input: { taskId: string; actorId: string; status: ProjectTaskStatus }): Promise<ProjectTask>;
}
...
export function createProjectAdministration(options: {
  ...
  return {
    list: (ownerId: string) => options.projects.list(ownerId),
    get: (id: string) => options.projects.getById(id),
    create: (input: { ownerId: string; name: string; description?: string }) =>
      options.projects.create(input),
    update: (input: { id: string; actorId: string; name?: string; description?: string; status?: ProjectStatus }) =>
      options.projects.update(input),
    tasks: (projectId: string) => options.projects.listTasks(projectId),
    createTask: (input: { projectId: string; actorId: string; title: string }) =>
      options.projects.createTask(input),
    updateTask: (input: { taskId: string; actorId: string; status: ProjectTaskStatus }) =>
      options.projects.updateTask(input),
  };
}
